...
function PaciBusterBeam:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 and not self.jackenstein then
        ...
        for i = 1, 8 do
            ...
            local z = SpareZ((i - 1) * 45, self.target_x, self.target_y)
            z.layer = self.layer - (0.01 * i)
            z.physics.speed = (self.bonus_anim and 20 or 15)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite("effects/rudebuster/beam", self.x, self.y)
